...
async function handleText(ev) {
  ...
  const classes = [...new Set((master.items || [])
...
async function handlePostback(ev) {
  ...
  if (step === "name") {
    ...
    if (items.length === 0)
      return reply(ev.replyToken, { type: "text", text: "該当データが見つかりません。" });
...
app.get("/diag", (req, res) => {
  ...
  if (items.length === 0) return res.json({ ok: false });
  const it = items[0];
  const v = pickVariant(it);
